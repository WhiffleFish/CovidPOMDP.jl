...
"""
struct CovidPOMDP{D<:Distribution} <: POMDP{CovidState, CovidAction, Int}
    symptom_dist::D
    Infdistributions::Vector{Gamma{Float64}}
...
- `discount::Float64=0.95`: POMDP discount factor
- `actions::Vector{CovidAction} = CovidAction.(0:0.1:1.0)`: Action space discretization
...
"""
function CovidState(I, params::CovidPOMDP, rng::AbstractRNG=Random.GLOBAL_RNG)::CovidState
    N = params.N
    ...
    return CovidState(S0, I0, R0, tests, CovidAction(0.0))
end
